...
Issues and specs for this repo live as GitHub issues on [`solanab/nsk`](https://github.com/solanab/nsk). Use the `gh`
...
Infer the repo from `git remote -v` — `gh` does this automatically when run inside a clone. Until the first push, pass
`-R solanab/nsk`.

## Pull requests as a triage surface

**PRs as a request surface: no.** *(Set to `yes` if this repo treats external PRs as feature requests; `triage` reads
this flag.)*
