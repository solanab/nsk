...
This goal is written to be used by subagents (via `spawn_subagent` or `execute-plan` workflow) to implement the 12
GitHub issues in vertical slices, with TDD at seams, and git commit after each issue completion.
...
Blocked by: none\
TDD seams: `internal/config/paths.go`, `internal/config/bind.go`, `internal/config/config.go`, `internal/app/app.go`
(error paths)\
...
Use `spawn_subagent` for each issue with prompt containing the issue body, current code, and "Implement per this goal,
TDD at seams, commit after done".
